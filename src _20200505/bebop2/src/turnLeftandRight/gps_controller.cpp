...
GpsController::GpsController(const ros::NodeHandle &nh): nh_(nh), has_home_geo_(false), reached_goal_(false), initial_gps_flag_(false), count_sway_(0), txt_open_flag(false)
{
    ...
    depth_command_sub_ = nh_.subscribe("/Controller/command", 1, &GpsController::depth_command_cb, this);
...
void GpsController::depth_command_cb(const std_msgs::Int8& command_msg)
{
...
void GpsController::check_reached_goal_only_position()
{
    ...
    if (target_position_.x < curr_position_.x)
        reached_goal_ = true; 
}

void GpsController::avoid()
{
