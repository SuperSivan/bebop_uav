...
bool GpsController::load_target_postition(const ros::NodeHandle &nh)
{
    ...
    found = found && nh.getParam("forwardSpeed", forwardSpeed);
    found = found && nh.getParam("objectYawThrehold", objectYawThrehold);
    found = found && nh.getParam("objectForwardSpeed", objectForwardSpeed);
    found = found && nh.getParam("objectRotateSpeed", objectRotateSpeed);
...
/**********************************类构建函数**********************************/
GpsController::GpsController(const ros::NodeHandle &nh): nh_(nh), has_home_geo_(false), reached_goal_(false), initial_gps_flag_(false), count_sway_(0), txt_open_flag(false),initial_yaw_(0)
{
    set_param();
...
void GpsController::orientation_control_cmd()
{
    ...
    if(last_avoidance_flag_==0&&avoidance_flag_==1)//转换时刻，更新target_yaw开始避障
        {
            target_position_.yaw=yaw_command_+yaw_command_*9/abs(yaw_command_);//加10度阈值
            //target_position_.yaw=yaw_command_;
            //std::cout<<target_position_.yaw<<std::endl;
            //std::cout<<"yaw_command_ is "<<yaw_command_*180/PI<<std::endl;
            //std::cout<<"target_position_ is "<<target_position_.yaw*180/PI<<std::endl;
            initial_yaw_=curr_position_.yaw;
        
        }
    last_avoidance_flag_=avoidance_flag_;
    curr_error_.z = target_position_.z - curr_position_.z;
    //curr_error_.yaw = yaw_command_-curr_position_.yaw;
    if(avoidance_flag_==1)
        curr_error_.yaw = target_position_.yaw-(curr_position_.yaw-initial_yaw_);
        //curr_error_.yaw = target_position_.yaw+curr_position_.yaw-initial_yaw_;
    else    
    ...
    if(avoidance_flag_==1)
        ROS_INFO("target_position_.yaw: %f", target_position_.yaw);
}
...
void GpsController::enforce_dynamic_constraints()
{
    ...
    if(avoidance_flag_)
    {
        ...
        if (vel_cmd_.Ayaw >= objectYawThrehold)//阈值原来为 8
        {
            cmd_Msg.linear.x = objectForwardSpeed;
            if (flag_1)
            {
                flag_1 = false;
                cmd_Msg.linear.y = -objectRotateSpeed;
            }
            else
            {
        ...
        else if (vel_cmd_.Ayaw <= -objectYawThrehold)
        {
            cmd_Msg.linear.x = objectForwardSpeed;
            if (flag_2)
            {
        ...
        else
        {
            flag_1 = true;
            flag_2 = true;
            cmd_Msg.linear.x = forwardSpeed;
            cmd_Msg.linear.y = 0;//由0.1改为0
        }        
       /*
