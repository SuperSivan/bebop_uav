...
const double uav_x = 0.9;
const double uav_y = 0.3;
...
public:
	...
	void set(double *angle_x)
	{
		...
		v = DEPTH_ROWS_MAX / 4;//ÓÉœÇ¶ÈŒÆËãÍŒÏñ×ø±êÏµÏÂË®Æœ·œÏòÉÏµÄvÖµ
...
void DepthAnalyse::depth_analyse(cv::Mat depth_c)
{
	try{
		...
		double target_yaw = p2p(target_x - current_x, target_y - current_y);//计算现在与目的的偏角
		
		//ROS_INFO("target_yaw: %f, current_yaw: %f", target_yaw, current_yaw);
		double bias_yaw = angle_substract(target_yaw, current_yaw);//计算所需要的转角
		...
		double temp = angtf.u;
		double initialBiasYaw =  bias_yaw;
		...
		{
			//ROS_INFO("obstacle!!!!!!!!!!");
			cv::putText(depth_c,"obstacle!!!!!!!!!!",Point(300,200),FONT_HERSHEY_SIMPLEX,2,Scalar(0),4,8);
			obstacle_ = true;
			condition = true;
			...
			while (safe)
			{
				while (left_ok)
				{
					leftYaw = tmp - step * step_size;
					step++;
					if (leftYaw < -50) left_ok = false;
					angtf.set(&leftYaw);
					yaw_dis = depth_c.at<uchar>(angtf.v, angtf.u);
					if (yaw_dis > N)
					{
						leftYaw = leftYaw  - step_size * 2;
						break;
					}
				}
				step = 1;
				while (right_ok)
				{
					rightYaw = tmp + step * step_size;
					step++;
					if (rightYaw > 50) right_ok = false;
					angtf.set(&rightYaw);
					yaw_dis = depth_c.at<uchar>(angtf.v, angtf.u);
					if (yaw_dis > N)
					{
						rightYaw = rightYaw + step_size * 2;
						break;
					}
				}
				...
				if(abs(leftYaw)>abs(rightYaw)) bias_yaw=rightYaw;//取偏航角小的，方便旋转
				else bias_yaw=leftYaw;

				if(left_ok||right_ok) //找到需要的转角
					break;
			}
		...
		cv::line(depth_c, Point(0, 60), Point(428, 60), Scalar(0), 2, CV_AA); 
		cv::line(depth_c, Point(angtf.u, 0), Point(angtf.u, 240), Scalar(0), 2, CV_AA);
		cv::line(depth_c, Point(temp, 0), Point(temp, 240), Scalar(155), 2, CV_AA);
		cv::putText(depth_c,std::to_string(initialBiasYaw),Point(temp,110),FONT_HERSHEY_SIMPLEX,1,Scalar(155),4,8);
		cv::putText(depth_c,std::to_string(bias_yaw),Point(angtf.u,110),FONT_HERSHEY_SIMPLEX,1,Scalar(0),4,8);
		//char save_path[256];
