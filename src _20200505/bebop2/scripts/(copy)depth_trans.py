...
ROOT_DIR = "/home/zhang/Project/MegaDepth/"
...
def test_simple(model, save_ori, img_ori):
    ...
    inv_depth_int = cv2.equalizeHist(inv_depth_int)
    inv_depth_int = cv2.resize(inv_depth_int, (640, 480), cv2.INTER_LINEAR)
    
    save_ori_img = save_ori + str(picnum).zfill(6) + '.jpg'
    save_depth_img = save_depth + str(picnum).zfill(6) + '.jpg'
    io.imsave(save_ori_img, img_ori)
    io.imsave(save_depth_img, inv_depth_int)
    return inv_depth_int
...
def callback(data):
    ...
    if count == 1:
        ...
        depth = cv2.resize(depth_Hist, (640, 480), cv2.INTER_LINEAR)
        #depth = filter(depth)
...
if __name__ == '__main__':
    ...
    save_ori = '/home/zhang/bebop_mega/'+ time.strftime('%b_%d_%Y_%H_%M_%S',time.localtime(time.time())) + '/'
    save_depth = save_ori + 'depth/'
